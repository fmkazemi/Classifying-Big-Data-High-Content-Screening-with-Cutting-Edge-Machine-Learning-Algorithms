...
proj_root = "/home/fmkazemi/code/preprocessing/"
...
#@info_csv = pd.read_csv(os.path.join(processed_path,'full_images%s.csv' % excludecontrol_app),index_col=0)
info_csv = pd.read_csv(os.path.join(processed_path,'full_images.csv'),index_col=0)
...
print("generating samples with size %d num %d seed %d" % (sample_size, num_samples, seed))
...
sampled_images = f.create_dataset("images", (images.shape[0] * num_samples, sample_size, sample_size, 3))
